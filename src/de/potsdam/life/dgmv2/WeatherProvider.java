...
import de.potsdam.life.dgmv2.ReadForecastTask.ForecastListener;
import de.potsdam.life.dgmv2.ReadLocationTask.LocationLoadedListener;
...
public class WeatherProvider extends AppWidgetProvider {
	...
	private String getZipcode(Context context) {
		...
		SharedPreferences preferredCitySharedPreferences = context
		...
		String zipcodeString = preferredCitySharedPreferences.getString(
				WeatherViewerActivity.PREFERRED_CITY_ZIPCODE_KEY, context
						.getResources().getString(R.string.default_zipcode));
	...
	private void startUpdateService(Context context) {
		...
		startServiceIntent.putExtra(
				context.getResources().getString(R.string.zipcode_extra),
				getZipcode(context));
	...
	public static class WeatherService extends IntentService implements
		...
		protected void onHandleIntent(Intent intent) {
			...
			zipcodeString = intent.getStringExtra(resources
					.getString(R.string.zipcode_extra));

			// load the location information in a background thread
			new ReadLocationTask(zipcodeString, this,
					new WeatherServiceLocationLoadedListener(zipcodeString))
					.execute();
